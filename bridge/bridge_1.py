"""
Padrão de projeto estrutural que tem a intenção de desacoplar 
uma abstração da sua implementação, de modo que as duas possam
variar e evoluir independentemente.
Abstração é uma camada de alto nível para algo, geralmente não 
faz nenhum trabalho por conta própria, ela delega parte ou todo 
o trabalho para a camada de implementação.
A diferença entre o Bridge e o Adapter está nas suas intenções, 
o padrão adapter faz as coisas funcionarem após terem sido projetadas,
o bridge faz funcionar antes que existam.
...
class IDevice(ABC):
    ...
    @volume.setter
    def volume(self, volume: int)-> int: pass
...
class TV(IDevice):
    ...
    # Essa TV é muito inteligente, quando ela tá desligada
    # ela avisa pra você ligar :)
    @volume.setter
...
if __name__ == '__main__':
    tv = TV()
    radio = Radio()
    remote = RemoteControlWithMute(tv)
